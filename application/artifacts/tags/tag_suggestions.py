...
def tags_by_team_and_tags(team, limit, tags):
    ...
    for _ in range(1, len(tags) + 1):
        results, meta = db.cypher_query(
            query,
            {"tags": tags, "match_count": 1, "limit": limit, "team_id": team.id},  # pylint:disable=unused-variable
        )
        _add_to_dictionary(results, tag_dictionary)

    sorted_results = sorted(tag_dictionary.items(), key=lambda collection: collection[1], reverse=True)[:limit]
