...
class ArtifactSearchBuilder:  # pylint:disable=too-few-public-methods
    ...
    SEARCH_CONFIG = {
        ...
        "label_prio": 2,
        "user_prio": 5,
        "synonyms_multiplier": 0.3
    }

    def __init__(self, team_id=None, offset=0, limit=10,  # pylint:disable=too-many-arguments
                 search="", synonyms="", date_range=None):
    ...
    def _tags_query(self):
        if self.search == "":
            return {"match_all": {}}
        tags_query = [self._tags_multi_match(self.search),
                      self._tags_multi_match(self.synonyms,
                                             boost=self.SEARCH_CONFIG['synonyms_multiplier'])]
    ...
    def _tags_multi_match(self, search, boost=1.0):
        return {"multi_match": {
            "query": search,
            "fields": [f"user_tags^{self.SEARCH_CONFIG['user_prio']}",
