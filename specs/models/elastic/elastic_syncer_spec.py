            ...
            with context("when artifact does not exist yet"):
                with before.each:
                    ...
                    with Mock() as elastic_mock:
                        elastic_mock.update(
                            doc_type=anything(), id=self.artifact.id_, index="artifact", body=anything()
        ...
        with context("a simple artifact"):
            with context("artifact exists"):
                with before.each:
                    ...
                    with Mock() as elastic_mock:
                        elastic_mock.delete(
                            doc_type=anything(), id=anything(), index="artifact", refresh="wait_for"
                        ).raises(NotFoundError)
                    current_app.es = elastic_mock
                    self.syncer.delete()
            ...
            with context("artifact does not exist"):
                with before.each:
                    ...
                    self.syncer = ElasticSyncer.for_artifact(self.artifact)
                    with Mock() as elastic_mock:
                        elastic_mock.delete(doc_type=anything(), id=anything(), index="artifact", refresh="wait_for")
                    current_app.es = elastic_mock
                    self.syncer.delete()
