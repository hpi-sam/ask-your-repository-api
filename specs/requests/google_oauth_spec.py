...
from specs.factories.google_oauth_mocks import (
    google_auth_patch,
    google_flow_patch,
    google_credentials_patch,
    google_revoke_access_patch,
...
with description("/users/<id>/oauth_providers/google") as self:
        ...
        with description("valid id_token"):
            with before.each:
                ...
                with google_auth_patch, google_client_id_patch_load, google_client_id_patch_open:
                    self.response = self.context.client().put(
                        f"/users/{user.id_}/oauths/google", data={"id_token": "test_id_token"}
                    )
            ...
            with it("responds with a user that includes google credentials"):
                expect(self.response.json).to(
                    have_key("google", {"has_offline_access": False, "user_id": google_user_data["sub"]})
                )
    ...
    with description("PATCH"):
        with description("valid auth_code"):
            with before.each:
                ...
                with google_flow_patch, google_credentials_patch:
                    self.response = self.context.client().patch(
                        f"/users/{user.id_}/oauths/google", data={"auth_code": "test_auth_code"}
                    )
            ...
            with it("responds with a user that includes google credentials"):
                expect(self.response.json).to(
                    have_key("google", {"has_offline_access": True, "user_id": google_user_data["sub"]})
                )
        ...
        with description("google not connected"):
            ...
            with it("responds with google oauth not found"):
                expect(self.response.json).to(have_key("error", "google oauth not found"))
    ...
    with description("/scopes DELETE"):
        with description("valid auth_code"):
            with before.each:
                ...
                with google_revoke_access_patch(status_code=200), google_credentials_patch:
                    self.response = self.context.client().delete(f"/users/{user.id_}/oauths/google/scopes")
        ...
        with description("google request access error"):
            ...
            with it("responds with could not revoke access"):
                expect(self.response.json).to(have_key("error", "could not revoke access"))
