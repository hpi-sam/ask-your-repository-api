...
with description("/") as self:
    ...
    with context("database mocked"):
        with before.each:
            with Mock() as elastic_mock:
                elastic_mock.search(ANY_ARG).returns({"hits": {"hits": []}})
        ...
        with it("returns database status on"):
            expect(self.response.json).to(have_keys({"service_name": "artefact service", "database_status": "on"}))
    ...
    with context("databse unavailable"):
        ...
        with it("returns database status off"):
            expect(self.response.json).to(have_keys({"service_name": "artefact service", "database_status": "off"}))
