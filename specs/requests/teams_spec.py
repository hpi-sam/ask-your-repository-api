...
with description("/teams") as self:
    ...
    with description("GET"):
        ...
        with it("responds with all my teams"):
            ...
            expect(self.response.json["teams"]).to(contain_only(have_key("name", "Blue"), have_key("name", "Red")))
    ...
    with description("POST"):
        with description("valid request"):
            with before.each:
                ...
                self.response = self.context.client().post("/teams", data={"name": "My Team"})
            ...
            with description("notify dialogflow adapter"):
                ...
                with it("notifies dialogflow adapter of team creation"):
                    ...
                    expect(json.loads(self.response_mock.calls[0].request.body.decode("utf8"))["name"]).to(
                        equal("My Team")
                    )
        ...
        with description("invalid request"):
            with before.each:
                ...
                self.response = self.context.client().post("/teams", data={"name": ""})
        ...
        with description("PUT"):
            with description("valid id"):
                with before.each:
                    ...
                    self.response = self.context.client().put(f"/teams/{self.team.id_}", data={"name": "Red"})
            ...
            with description("invalid id"):
                with before.each:
                    ...
                    team.delete()
                    self.response = self.context.client().put(f"/teams/{team.id_}", data={"name": "Blue"})
            ...
            with description("invalid request"):
                with before.each:
                    team = Team(name="Red").save()
                    self.response = self.context.client().put(f"/teams/{team.id_}", data={"name": ""})
