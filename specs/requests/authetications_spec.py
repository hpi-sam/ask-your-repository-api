...
from specs.factories.google_oauth_mocks import (
    google_auth_patch,
    google_client_id_patch_load,
...
with description("/authentications") as self:
    ...
    with description("POST or login"):
        with before.each:
            self.user = User(username="TestUser", email="test@example.com", password="test").save()

        with description("with email and password"):
            with before.each:
                self.response = self.context.client().post(
                    "/authentications", data={"email_or_username": "test@example.com", "password": "test"}
                )
        ...
        with description("with username and password"):
            with before.each:
                self.response = self.context.client().post(
                    "/authentications", data={"email_or_username": "TestUser", "password": "test"}
                )
        ...
        with description("with username and password and set_cookies set to false"):
            with before.each:
                self.response = self.context.client().post(
                    "/authentications", data={"email_or_username": "TestUser", "password": "test", "set_cookies": False}
                )
            ...
            with google_client_id_patch_load, google_client_id_patch_open:
                with description("with existing user"):
                    with before.each:
                        ...
                        with google_auth_patch, google_client_id_patch_load, google_client_id_patch_open:
                            self.response = self.context.client().post(
                                "/authentications", data={"id_token": "test_id_token"}
                            )
                    ...
                    with it("returns the user with google auth"):
                        expect(self.response.json).to(
                            have_key("google", {"user_id": google_user_data["sub"], "has_offline_access": False})
                        )
        ...
        with description("with invalid username or email"):
            with before.each:
                self.response = self.context.client().post(
                    "/authentications", data={"email_or_username": "WrongUsername", "password": "test"}
                )
        ...
        with description("with invalid password"):
            with before.each:
                self.response = self.context.client().post(
                    "/authentications", data={"email_or_username": "TestUser", "password": "wrong_password"}
                )
        ...
        with description("with password missing"):
            with before.each:
                self.response = self.context.client().post("/authentications", data={"email_or_username": "TestUser"})

            with it("responds with 422"):
                expect(self.response.status_code).to(equal(422))

            with it("returns an error message"):
                expect(self.response.json).to(equal({"errors": {"password": ["missing value for required field"]}}))

        with description("with no params missing"):
            with before.each:
                self.response = self.context.client().post("/authentications")
            ...
            with it("returns an error message"):
                expect(self.response.json).to(
                    equal({"errors": {"login_params": ["id_token or email_or_username missing"]}})
                )
    ...
    with description("DELETE or logout"):
        with description("when logged in"):
            with before.each:
                ...
                user_response = self.context.client().post(
                    "/authentications", data={"email_or_username": "TestUser", "password": "test"}
                )
                token = user_response.json["csrf_token"]

                self.response = self.context.client().delete("/authentications", headers={"X-CSRF-TOKEN": token})
            with it("responds with 200"):
                expect(self.response.status_code).to(equal(200))
        ...
        with description("without being logged in"):
            with before.each:
                self.response = self.context.client().delete("/authentications")
            ...
            with it("returns message not allowed"):
                expect(self.response.json).to(
                    equal(
                        {
                            "msg": 'Missing JWT in headers or cookies (Missing cookie "access_token_cookie"; '
