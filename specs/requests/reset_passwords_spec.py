    ...
    with description("PUT"):
        with description("valid jwt"):
            with before.each:
                ...
                self.response = self.context.client().put(
                    "/password_resets",
                    data={"reset_token": reset_password_token(self.user), "password": "new_password"},
                )
        ...
        with description("invalid jwt"):
            with before.each:
                self.user = UserFactory.create_user()
                self.response = self.context.client().put(
                    "/password_resets", data={"reset_token": "invalid_jwt", "password": "new_password"}
                )
        ...
        with description("expired jwt"):
            with before.each:
                ...
                self.response = self.context.client().put(
                    "/password_resets",
                    data={"reset_token": expired_reset_password_token(self.user), "password": "new_password"},
                )
        ...
        with description("already used jwt"):
            with before.each:
                ...
                self.context.client().put("/password_resets", data={"reset_token": token, "password": "new_password"})
                self.response = self.context.client().put(
                    "/password_resets", data={"reset_token": token, "password": "another_new_password"}
                )
